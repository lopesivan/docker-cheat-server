...
class LearnXYAdapter(object):
    ...
    def __init__(self):
        ...
        if "Comments" in self._topics_list:
            self._topics_list = [
                x for x in self._topics_list if x != "Comments"] + ["Comments"]
...
class LearnClojureAdapter(LearnXYAdapter):
    ...
    def _is_block_separator(self, before, now, after):
        if (re.match(r'\s*$', before)
            ...
            block_name = '_'.join([x.strip(",&:")
                                  for x in block_name.strip(", ").split()])
            return block_name
        return None
...
class LearnMapleAdapter(LearnXYAdapter):
    "Learn Maple in Y Minutes"
    prefix = "maple"
    _filename = "maple.html.markdown"
    _splitted = False
...
_ADAPTERS = {cls.prefix: cls()
             for cls in vars()['LearnXYAdapter'].__subclasses__()}
